...
\subsection{Graphs}
A simplicial complex $\sc$ whose elements are all $0$- or $1$-simplices is called a \define{graph}.\marginnote{The whole goal of Section 1 was to build the intuition that simplices are $n$-dimensional triangles; however, we need to let go of the idea that these sets are embedded in a higher-dimensional ``space''. Instead, we're \emph{building} ``spaces'' by gluing simplices.} The idea is here is that if $\se_{12}=\{\se_1,\se_2\}$ is a line segment in $\sc$, then its ``endpoints'' $\se_1$ and $\se_2$ must also be in $\sc$. 
\begin{exercise}
